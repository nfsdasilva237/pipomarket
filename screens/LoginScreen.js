// screens/LoginScreen.js - VERSION MODERNE AVEC CODE STARTUP
...
export default function LoginScreen({ navigation }) {
  ...
  // Mode de connexion : 'email' ou 'code'
  ...
  // Connexion avec code startup
  const handleStartupCodeLogin = async () => {
    if (!startupCode.trim()) {
      Alert.alert('Erreur', 'Veuillez entrer votre code d\'accès');
      return;
    }
    ...
    try {
      // 1. D'abord créer une session anonyme Firebase Auth
      // Cela permet d'avoir request.auth != null dans les règles Firestore
      await signInAnonymously(auth);
      console.log('✅ Session anonyme créée');

      // 2. Rechercher la startup avec ce code d'accès
      const startupsRef = collection(db, 'startups');
      const q = query(startupsRef, where('accessCode', '==', startupCode.trim().toUpperCase()));
      const querySnapshot = await getDocs(q);

      if (querySnapshot.empty) {
        Alert.alert('Code invalide', 'Aucune startup trouvée avec ce code d\'accès');
        return;
      }
      ...
      if (startupData.active === false) {
        Alert.alert('Accès refusé', 'Cette startup a été désactivée. Contactez l\'administrateur.');
        return;
      }

      // Navigation directe vers le dashboard startup
      Alert.alert(
        'Bienvenue !',
        `Connexion réussie à ${startupData.name}`,
        [
          {
            text: 'Continuer',
            onPress: () => {
              navigation.replace('StartupDashboard', { 
                ...
                isCodeAccess: true // Indique que c'est un accès par code
              });
            }
          }
        ]
      ...
      console.error('Erreur connexion code:', error);
        ...
        >
          ...
          <View style={styles.modeSelector}>
            ...
            >
              <Text style={[
                styles.modeButtonText,
                loginMode === 'code' && styles.modeButtonTextActive
              ]}>
                🔑 Code Startup
              </Text>
            </TouchableOpacity>
            ...
            {loginMode === 'email' ? (
              ...
              <>
                ...
                <View style={styles.codeSection}>
                  <Text style={styles.codeSectionTitle}>🏢 Accès Startup</Text>
                ...
                <View style={styles.inputContainer}>
                  ...
                  <TextInput
                    ...
                    placeholder="STARTUP-XXXXXX"
                    placeholderTextColor="#999"
                    value={startupCode}
                    onChangeText={(text) => setStartupCode(text.toUpperCase())}
                    autoCapitalize="characters"
                    editable={!loading}
                  />
                </View>

                {/* BOUTON CONNEXION CODE */}
                <TouchableOpacity
                  style={[styles.loginButton, styles.codeLoginButton, loading && styles.loginButtonDisabled]}
                  onPress={handleStartupCodeLogin}
                  disabled={loading}
                >
                  {loading ? (
                    <ActivityIndicator color="white" size="small" />
                    ...
                    <>
                      <Text style={styles.loginButtonText}>Accéder à ma startup</Text>
                      <Text style={styles.loginButtonIcon}>→</Text>
                    </>
                  )}
                </TouchableOpacity>
                ...
                <View style={styles.codeInfo}>
                  <Text style={styles.codeInfoText}>
                    💡 Vous avez reçu un code de l'administrateur ? Entrez-le ci-dessus pour accéder directement à votre tableau de bord startup.
                  </Text>
                </View>
              </>
            )}
