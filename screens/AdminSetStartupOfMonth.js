...
export default function AdminSetStartupOfMonth({ navigation }) {
  ...
  const renderStartupLogo = (startup, size = 40) => {
    if (isImageUrl(startup.logo)) {
      return <Image source={{ uri: startup.logo }} style={{ width: size, height: size, borderRadius: size / 2 }} resizeMode="cover" />;
    }
    return <Text style={{ fontSize: size * 0.8 }}>{startup.logo || '🏪'}</Text>;
  };
        ...
        {selectedStartup && (
          <View style={styles.selectedSection}>
            ...
            <View style={styles.selectedCard}>
              <View style={styles.selectedLogoBox}>
                {renderStartupLogo(selectedStartup, 50)}
              </View>
...
const styles = StyleSheet.create({
  ...
  selectedLogoBox: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#f0f0f0', justifyContent: 'center', alignItems: 'center', marginRight: 16, overflow: 'hidden' },
  selectedInfo: { flex: 1 },
  ...
  startupLogoBox: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#f0f0f0', justifyContent: 'center', alignItems: 'center', marginRight: 16, overflow: 'hidden' },
  startupInfo: { flex: 1 },
